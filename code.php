    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateUsername'])) {
        ...
        if ($row) {
            ...
            if ($currentPassword === $storedPassword) {
                ...
                $usernameCheckQuery = "SELECT * FROM yamiraku_staff WHERE staffUsername = ?";
                $stmt = mysqli_prepare($db, $usernameCheckQuery);
                ...
                if (mysqli_num_rows($result) === 0) {
                    ...
                    $updateQuery = "UPDATE yamiraku_staff SET staffUsername = ? WHERE StaffID = ?";
                    $stmt = mysqli_prepare($db, $updateQuery);
                    mysqli_stmt_bind_param($stmt, "si", $newUsername, $staffID);
                    ...
                    echo '<script>alert("The chosen username is already taken. Please choose another username.");</script>';
                    echo '<script>window.location = "setting.php";</script>';
                }
            } else {
...
if (isset($_POST['updateApplication'])) {
    ...
    if (!empty($_FILES['resumePath']['name'])) {
        $targetPath = "uploads/"; // Specify the directory where you want to save the uploaded files
